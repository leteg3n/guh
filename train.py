...
# Paths
DIR = 'C:/Fuck one drive local stuff/hw5'
...
model_dir = os.path.join(DIR, '/Histopathologic-Cancer-Detection-master/resnet34')
model_name = 'resnet34'
n_groups = 15
b_size = 96
...
# Merge train data with patch ids
...
folds_id_val = []
folds_label_val = []
for train_index, test_index in skf.split(train_data_sampled['id'].values, train_data_sampled['label'].values, train_data_sampled['wsi'].values):
    folds_id_train.append(train_data_sampled['id'].values[train_index])
    folds_id_val.append(train_data_sampled['id'].values[test_index])
    folds_label_train.append(train_data_sampled['label'].values[train_index])
    folds_label_val.append(train_data_sampled['label'].values[test_index])
...
for valid_idx in range(n_groups):
    ...
    base_model = pretrainedmodels.resnet34(num_classes=1000, pretrained='imagenet').to(device)
